...
GO
-- ============================================================================
-- PR_tbConceptosPago_Find
-- Tipo: FIND - Con IDs y descripciones, sin auditoría
-- ============================================================================
ALTER PROCEDURE [finanza].[PR_tbConceptosPago_Find]
    @Cpa_Id int
AS
BEGIN
    SET NOCOUNT ON;
...
BEGIN
    ...
    SELECT
        ...
        epa.Epa_Descripcion AS DescripcionEstadoPago,
        ...
        -- Campos de auditoría
        cco.Per_EsEliminado,
        cco.Per_UsuarioRegistra,
        usuarioRegistra.Usu_Name AS NombreCompletoUsuarioRegistra,
        cco.Per_FechaRegistra,
        cco.Per_UsuarioModifica,
        usuarioModificacion.Usu_Name AS NombreCompletoUsuarioModifica,
        cco.Per_FechaModifica
    FROM
        finanza.tbCuentasCobrar cco
        INNER JOIN finanza.tbConceptosPago cp ON cco.Cpa_Id = cp.Cpa_Id
        INNER JOIN finanza.tbEstadosPago epa ON cco.Epa_Id = epa.Epa_Id
        INNER JOIN app.tbAlumnos a ON cco.Alu_Id = a.Alu_Id
        INNER JOIN app.tbPersonas per ON a.Per_Id = per.Per_Id
        -- JOINs para auditoría
        LEFT JOIN seguridad.tbUsuarios AS usuarioRegistra
            ON cco.Per_UsuarioRegistra = usuarioRegistra.Usu_Id
        LEFT JOIN seguridad.tbUsuarios AS usuarioModificacion
            ON cco.Per_UsuarioModifica = usuarioModificacion.Usu_Id
    WHERE
        cco.Cco_Id = @Cco_Id
        AND cco.Per_EsEliminado != 1;
END
GO
...
BEGIN
    ...
    FROM
        ...
        INNER JOIN app.tbPersonas per ON a.Per_Id = per.Per_Id
    WHERE
        cco.Per_EsEliminado != 1
    ORDER BY
        cco.Cco_FechaVencimiento DESC,
        cco.Cco_Id DESC;
END
GO
...
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [finanza].[PR_tbEstadosPago_Delete]
    @Epa_Id int,
...
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [finanza].[PR_tbEstadosPago_Insert]
    @Epa_Descripcion nvarchar(50),
...
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [finanza].[PR_tbEstadosPago_Update]
    @Epa_Id int,
    @Epa_Descripcion nvarchar(50),
    @Epa_UsuarioModifica int
AS
...
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [finanza].[PR_tbPagosDetalle_Delete]
    @Pde_Id int,
    @Per_UsuarioModifica int
AS
BEGIN
...
-- ============================================================================
ALTER PROCEDURE [finanza].[PR_tbRecibos_Detail]
    @Rec_Id int
AS
BEGIN
    SET NOCOUNT ON;
