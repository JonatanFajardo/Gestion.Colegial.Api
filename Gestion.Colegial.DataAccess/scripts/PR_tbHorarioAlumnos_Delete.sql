-- =====================================================================
-- Procedure: PR_tbHorarioAlumnos_Delete
-- Descripción: Elimina (físicamente) un horario de alumno
-- Fecha: 2025-11-27
-- =====================================================================

USE [DB_GestionColegial]
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[app].[PR_tbHorarioAlumnos_Delete]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [app].[PR_tbHorarioAlumnos_Delete]
GO

CREATE PROCEDURE [app].[PR_tbHorarioAlumnos_Delete]
    @HoAl_Id INT
AS
BEGIN
    SET NOCOUNT ON;

    BEGIN TRY
        BEGIN TRANSACTION;

        DELETE FROM [app].[tbHorarioAlumnos]
        WHERE [HoAl_Id] = @HoAl_Id;

        COMMIT TRANSACTION;

        SELECT 1 AS CodeResult; -- Éxito
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION;

        SELECT 0 AS CodeResult; -- Error
    END CATCH
END
GO
