-- 040b_DashboardConsulta_Fix_RS2.sql
-- Fix: RS2 usaba ROW_NUMBER() que produce BIGINT; Anio quedaba Int64 en DataTable
-- lo que causaba InvalidCastException en Mapear.Convert al asignar a int? (Int32).
-- Solucion: reemplazar CTE con UNION ALL + CAST(@Anio AS INT) explícito.
...
BEGIN
    ...
    -- RS2: Evolución Matrícula (últimos 5 años) — CAST explícito a INT para evitar BIGINT
    SELECT CAST(@Anio   AS INT) AS Anio, ISNULL((SELECT COUNT(*) FROM [app].[tbAlumnos] WHERE AnioCursado = @Anio),   0) AS TotalAlumnos
    UNION ALL
    SELECT CAST(@Anio-1 AS INT), ISNULL((SELECT COUNT(*) FROM [app].[tbAlumnos] WHERE AnioCursado = @Anio-1), 0)
    UNION ALL
    SELECT CAST(@Anio-2 AS INT), ISNULL((SELECT COUNT(*) FROM [app].[tbAlumnos] WHERE AnioCursado = @Anio-2), 0)
    UNION ALL
    SELECT CAST(@Anio-3 AS INT), ISNULL((SELECT COUNT(*) FROM [app].[tbAlumnos] WHERE AnioCursado = @Anio-3), 0)
    UNION ALL
    SELECT CAST(@Anio-4 AS INT), ISNULL((SELECT COUNT(*) FROM [app].[tbAlumnos] WHERE AnioCursado = @Anio-4), 0)
    ORDER BY Anio;
    ...
    -- RS10: Indicadores Institucionales
...
PRINT N'PR_DashboardConsulta_KPIs corregido: RS2 usa UNION ALL + CAST INT (fix BIGINT).';
